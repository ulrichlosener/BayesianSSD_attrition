...
We can see that the effect sizes for treatments a), b), c), and d) are equal to $\delta_a=0.35$, $\delta_b=0.26$, $\delta_c=-0.24$, and $\delta_d=-0.51$, respectively. Positive (negative) effect sizes indicate an increase (decline) of perceived severity over time. Thus, in condition a) and b), the perceived severity increases over time, while it decreases in conditions c) and d). For an illustrative overview of the influence of the multilevel model parameters on the growth curves, please visit the [ShinyApp](https://losener.shinyapps.io/MLM_input2/) developed for this purpose.
...
The output informs us that the minimal sample size needed to achieve a power of at least $\eta=.80$ when testing $\mathscr{H}_1$ against its complement $\mathscr{H}_c$ is $\mathbf{N_{total}=684}$, while taking the anticipated attrition pattern into account. 
...
This tells us the sample sizes that were evaluated during the binary search (775, 412, 593, 684) along with their resulting power, the proportion of models that had to be simplified due to high attrition (0%), the number of iterations (4) and the runtime of the SSD procedure in minutes (19). While this information is not essential for the result of the SSD, it can be useful for gaining more insight into the SSD algorithm.
